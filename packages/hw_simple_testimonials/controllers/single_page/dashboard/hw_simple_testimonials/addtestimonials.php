...
class Addtestimonials extends DashboardPageController {
	...
	public function edit($sID)
	{
		...
		$db = Loader::db();
		$q = "SELECT sID, author, created_at, company, testimonial, extra FROM bthwsimpletestimonial WHERE sID = '{$sID}'";
		$r = $db->query($q);
		if ($r) {
			 $row = $r->fetchRow();

			$this->set('hwAuthor', $row['author']);
            $this->set('hwCreatedAt', $row['created_at']);
	...
	public function add_testimonials()
	{
        ...
        if (strlen($CreatedAt) > 255) {
			$this->error->add(t('The Created At field has too many characters'));
        }
